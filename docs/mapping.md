...
6. Always Delete Object Maps When Data is Deleted: when data in either the WordPress or Salesforce object type for a fieldmap is deleted, the plugin will check for object maps connected to the record that was deleted even if the delete action trigger is not checked. If it finds those object maps, they will be deleted.
7. Process Asynchronously: whether push data will be sent to Salesforce immediately, or when `wp_cron` runs.
