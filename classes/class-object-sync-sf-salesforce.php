...
class Object_Sync_Sf_Salesforce {
	...
	public function is_authorized() {
		...
		if ( $installed_site_root !== $options_site_root ) {
			...
			$this->force_expire_authorization();
			return false;
		}
	...
	protected function refresh_token() {
		...
		if ( 200 !== $response['code'] ) {
			$this->force_expire_authorization( $response );
		}
	...
	/**
	 * Force the authorization to expire
	 * This happens in cases where the token cannot be renewed, OR where the current WordPress domain name
	 * does not match the domain where the plugin is authorized.
	 * Ideally, we could also detect the Salesforce instance domain and run this, but this does not currently happen.
	 *
	 * @param array $response optional array of response from Salesforce API call.
	 */
	protected function force_expire_authorization( $response = array() ) {
		...
		$title = esc_html__( 'The plugin authorization has expired and needs to be renewed.', 'object-sync-for-salesforce' );
		...
		if ( ! empty( $response ) ) {
			...
			$body = esc_html__( '<p>The WordPress URL where Salesforce was originally installed does not match the URL where WordPress is currently running. This is generally a sign that the plugin has been moved, and is likely to be using a different Salesforce instance. Reauthorize the connection to Salesforce to begin using it again.', 'object-sync-for-salesforce' );
		}
